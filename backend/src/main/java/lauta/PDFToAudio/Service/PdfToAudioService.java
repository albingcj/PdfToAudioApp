...
public class PdfToAudioService {
    ...
    private static final Logger logger = LoggerFactory.getLogger((PdfToAudioService.class));

    // URL of the Python Flask API
    private static final String PYTHON_API_URL = "http://pdftoaudio-python-script:5000"; // Replace with the actual service name
    ...
    public String pdfToString(File file) {
        ...
        public int textToSpeech(File file, String uuid) {
            ...
            if (content == null || content.trim().isEmpty()) {
                throw new PdfProcessingException("The extracted text from the PDF is empty");
            }
            ArrayList<String> chunks = largeTextToChunks(file);
            String pathString = Paths.get(System.getProperty("user.dir"), "outputs").toString();
    ...
    private void callPythonGenerate(String chunk, int index) throws IOException, InterruptedException {
        HttpClient client = HttpClient.newHttpClient();
        String url = PYTHON_API_URL + "/generate";
        String jsonBody = String.format("{\"text\": \"%s\", \"chunk_index\": %d}", chunk.replace("\"", "\\\""), index);
        ...
        if (response.statusCode() != 200) {
            throw new IOException("Python script failed with status code: " + response.statusCode() + ", message: " + response.body());
        }

        logger.info("Python script successfully processed chunk {}: {}", index, response.body());
    }

    private void combineAudioFiles(String uuid) throws IOException, InterruptedException {
        HttpClient client = HttpClient.newHttpClient();
        ...
        logger.info("Python script successfully combined audio files for UUID: {}", uuid);
    }
    ...
    public InputStream downloadAudioFile(String uuid) throws IOException {
        ...
        if (!Files.exists(filePath)) {
            throw new IOException("The file with UUID: " + uuid + " could not be found");
        }
